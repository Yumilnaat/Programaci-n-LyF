...
lista_filtrada = list(filter(lambda pelicula: pelicula["Director"]=="Christopher Nolan", peliculas))

nueva_lista = list(map(lambda pelicula: {"titulo": pelicula["Titulo"], "calificacion": pelicula["CalificaciónIMDB"]}, lista_filtrada))

promedio_nolan = reduce(lambda acumulador, pelicula: acumulador + float(pelicula["CalificaciónIMDB"]), lista_filtrada, 0) / len(lista_filtrada)

print(nueva_lista)
print("Promedio de calificación de Christopher Nolan:", promedio_nolan)
